...
class KoBERTEmbeddingLayer(Layer):
    def __init__(self, bert_model, **kwargs):
        super(KoBERTEmbeddingLayer, self).__init__(**kwargs)
    ...
    def get_config(self):
        ...
        config = super(KoBERTEmbeddingLayer, self).get_config()
...
class Proposed(BaseModel):
            ...
            for index, row in df.iterrows():
                ...
                for category in PARAMS.FEATURES:
                    ...
                    if PARAMS.FULL_FEATURES[category] == 'str':
                        ...
                        PARAMS.SENTENCE_FEATURE = len(stat_feature___)
                        
                        for f in stat_feature___:  # , "_relevance_score"
                            c_name = category + f
    ...
    def build(self):
        ...
        #bert_model = TFDistilBertModel.from_pretrained('distilbert-base-uncased')
        bert_model = TFBertModel.from_pretrained('monologg/kobert')
        shared_embedding_layer = KoBERTEmbeddingLayer(bert_model=bert_model)
        ...
        sentence_attention_layer = MultiHeadAttention(
            num_heads=PARAMS.NUM_HEADS, key_dim=PARAMS.PROJ_DIM // PARAMS.NUM_HEADS,seed=42
        )
        ...
        output = Dense(64, activation='relu', kernel_initializer=HeNormal())(output)
    ...
    def test(self):
        ...
        self.model = load_model(
            PARAMS.MODEL_PATH,
            safe_mode=False,
            custom_objects={'KoBERTEmbeddingLayer': KoBERTEmbeddingLayer,
