...
# === KoBERT 사용을 위한 AutoTokenizer와 TFAutoModel 임포트 ===
...
class BERTEmbeddingLayer(Layer):
    ...
    def from_config(cls, config):
        # KoBERT 모델 로딩 (TensorFlow 버전) – pretrained 모델 이름 변경됨
        bert_model = TFAutoModel.from_pretrained('skt/kobert-base-v1')
...
class Bert(BaseModel):
    ...
    def build(self):
       # === KoBERT 사용: TFAutoModel를 통해 KoBERT 로딩 ===
        bert_model = TFAutoModel.from_pretrained('skt/kobert-base-v1')
    ...
    def test(self):
        #self.model = load_model('model.keras', custom_objects={'BERTEmbeddingLayer': BERTEmbeddingLayer}, compile=False)
        #self.model = load_model('model.h5', custom_objects={'BERTEmbeddingLayer': BERTEmbeddingLayer}, compile=False)
        self.model = keras.saving.load_model("KoBert_model.keras", custom_objects={'BERTEmbeddingLayer': BERTEmbeddingLayer})
        super().test()
