...
#from keras.src.models import save_model
...
class BaseModel(ABC):
    ...
    def train(self):
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices((tuple(self.train_inputs), self.train_labels))
        train_dataset = train_dataset.shuffle(len(self.train_labels))  # Shuffle data
        train_dataset = train_dataset.batch(PARAMS.BATCH_SIZE)  # Apply batching
        train_dataset = train_dataset.prefetch(tf.data.experimental.AUTOTUNE)  # Prefetch for faster loading

        val_dataset = tf.data.Dataset.from_tensor_slices((tuple(self.val_inputs), self.val_labels))
        val_dataset = val_dataset.batch(PARAMS.BATCH_SIZE)  # Only batch, no shuffle needed
